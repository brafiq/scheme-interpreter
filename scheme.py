...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if not scheme_procedurep(procedure):
            raise SchemeError('cannot call {0}'.format(procedure))
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if formals is nil and vals is nil:
            return child
        if (formals.second is nil and vals.second is not nil) or (vals.second is nil and formals.second is not nil):
            raise SchemeError('number of arg values does not match formal params')
        while formals is not nil and vals is not nil and formals.second is not nil and vals.second is not nil:
            if (vals is nil and formals is not nil) or (formals is nil and vals is not nil) or (vals.second is nil and formals.second is not nil) or (formals.second is nil and vals.second is not nil):
                raise SchemeError('number of arg values does not match formal params')
